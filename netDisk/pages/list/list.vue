				...
				<scroll-view scroll-y="true" class="flex-1">
					<template v-if="index === 0">
						...
						<f-list v-for="(item,index) in downing" :key="'i' + index" :item="item" :index="index">
							...
							<progress slot="bottom" :percent="item.download" activeColor="#6c9ef4" :stroke-width="4"></progress>
						</f-list>
					...
					<template v-else>
						<view class="border-bottom border-light-secondary font p-2 text-muted">
							上传中({{ uploading.length }})</view>
						<view class="p-2 border-bottom border-light-secondary font text-muted">
						...
						<f-list v-for="(item,index) in uploading" :key="'i' + index" :item="item" :index="index">
							...
							<!-- 进度条时间，percent属性绑定下载百分比 -->
							<progress slot="bottom" :percent="item.progress" activeColor="#6c9ef4" :stroke-width="4"></progress>
						</f-list>
						
						<view class="p-2 border-bottom border-light-secondary font text-muted">
							上传完成({{ uploaded.length}}) 
						</view>
						<f-list v-for="(item,index) in uploaded" :key="'d' + index" :item="item" :index="index" :showRight="false">
						</f-list>
					</template>
				</scroll-view>
			</swiper-item>
